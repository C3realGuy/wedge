...
function Like()
{
	...
	else
	{
		...
		else
		{
			...
			//'SELECT id_topic, id_member, sssubject, body' // Hide
			$request = wesql::query('
			...
			if (wesql::num_rows($request) != 0)
			{
				//list ($id_topic, $id_author,ss $subject, $body) = wesql::fetch_row($request); // Hide: add $body
	...
	//if ((!$contains_hide || $hide_force_allow_dislike) && $row = wesql::fetch_row($request)) // Hide: Only dislike if it doesn't contain a hide or we are special permissions
	...
	else //if($is_liked === false) // Hide: Check if is_liked, otherwise we will like again
	{
